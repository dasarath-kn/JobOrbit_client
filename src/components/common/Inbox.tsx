...
import { conversation, getmessages, getUserdata } from '../../Api/userApi';
        ...
        <div className='w-full  flex flex-row  h-5/6 rounded-xl'>
          <div className='p-3 lg:w-96 w-36 shadow-2xl rounded-xl  border-gray-200  space-y-3'>
            ...
            <div className='flex justify-evenly  flex-col md:flex-row lg:flex-row w-full h-11 text-white'>
              ...
              <button onClick={() => { setSelected(true), setRole('user') }} className={`w-full h-11 text-black ${!selected ? '  font-semibold' : 'text-xl border-b-2   font-semibold'}`}>People</button>
              <button onClick={() => { setSelected(false), setRole('company') }} className={`w-full h-11 text-black ${selected ? '  font-semibold' : 'text-xl border-b-2  font-semibold'}`}>Companies</button>
            </div>
